...
let dataGlobal;  // holds entire CSV
...
d3.csv("merged_dataset.csv", d3.autoType).then(raw => {
  ...
  initCountrySelects();
  initGroupSelect();
  initYearSelect();     // populate based on default group, default year = 2022
...
// ── Initialize Group dropdown dynamically ─────────────────────────────────
function initGroupSelect() {
  const sel = d3.select("#groupSelect");
  sel.html("");
...
// ── Compute valid years for a selected group's metrics ────────────────────
...
// ── Initialize or re-populate the Year dropdown ───────────────────────────
function initYearSelect() {
  const grp = d3.select("#groupSelect").property("value");
  ...
  // default to 2022 if available, else fallback to most recent
...
function attachEventListeners() {
  ...
  d3.select("#groupSelect").on("change", () => {
    initYearSelect();
    updateChart();
  });
  d3.select("#yearSelectRadar").on("change", updateChart);
}
...
function updateChart() {
  ...
  const year          = +d3.select("#yearSelectRadar").property("value");

  const allAxes    = groups[selectedGroup];
  const totalAxes  = allAxes.length;
  const angleSlice = (Math.PI * 2) / totalAxes;

  let data = dataGlobal.filter(d => d.Year === year);
